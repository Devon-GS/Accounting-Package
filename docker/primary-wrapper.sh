...
# Trap termination signals to ensure graceful shutdown
trap 'echo "Caught signal, stopping postgres..."; kill -TERM $postgres_pid; wait $postgres_pid; exit 0' SIGINT SIGTERM
...
# NOW it is safe to modify pg_hba.conf because initialization is complete
