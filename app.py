...
REPORT_LINKED_DESCRIPTION_OPTIONS = [
	("shop_cost_of_sale_bank_eft", "Shop cost of sale Bank eft"),
	("shop_cost_of_sale_payout_from_till", "Shop cost of sale payout from till"),
	("cost_of_sale_fuel", "Cost of sale fuel"),
	("general_expenses", "General expenses"),
	("accountant_fee", "Accountant fee"),
	("bank_chargers", "Bank chargers"),
	("insurance", "Insurance"),
	("salaries_and_wages", "Salaries and wages"),
	("staff_fuel", "Staff Fuel"),
	("staff_training_uniform_entertainment", "Staff training,uniform & Entertainment"),
	("sars_paye_uif", "SARS- PAYE UIF"),
	("mibco", "Mibco"),
	("rma_workmans_compensation", "RMA Workmans compensation"),
	("electricity", "Electricity"),
	("telephone_mtn_vodacom", "Telephone MTN & VODACOM"),
]
REPORT_FIXED_DESCRIPTION_OPTIONS = [
	("rent", "Rent"),
	("water", "Water"),
	("training", "Training"),
]
REPORT_DESCRIPTION_OPTIONS = [
	*REPORT_LINKED_DESCRIPTION_OPTIONS[:13],
	*REPORT_FIXED_DESCRIPTION_OPTIONS,
	*REPORT_LINKED_DESCRIPTION_OPTIONS[13:],
]
REPORT_LINKED_DESCRIPTION_KEYS = {key for key, _ in REPORT_LINKED_DESCRIPTION_OPTIONS}
REPORT_FIXED_DESCRIPTION_KEYS = {key for key, _ in REPORT_FIXED_DESCRIPTION_OPTIONS}
REPORT_DESCRIPTION_KEYS = {key for key, _ in REPORT_DESCRIPTION_OPTIONS}
...
def sqlite_schema_statements() -> list[str]:
	return [
		...
		""",
		"""
		CREATE TABLE IF NOT EXISTS report_account_links (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			description_key TEXT NOT NULL,
			account_id INTEGER NOT NULL,
			created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
			UNIQUE (description_key, account_id),
			FOREIGN KEY (account_id) REFERENCES accounts (id) ON DELETE CASCADE
		);
		""",
		"""
		CREATE TABLE IF NOT EXISTS report_fixed_amounts (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			description_key TEXT NOT NULL,
...
def report_linked_amounts_for_month(month_value: str | None) -> dict[str, float]:
	if not month_value:
		return {}
	rows = query_all(
		"""
		SELECT
			report_account_links.description_key AS description_key,
			SUM(payments.amount_paid) AS total_amount
		FROM report_account_links
		JOIN payments ON payments.account_id = report_account_links.account_id
		WHERE substr(payments.payment_date, 1, 7) = ?
		GROUP BY report_account_links.description_key
		""",
		(month_value,),
	)
	return {row["description_key"]: float(row["total_amount"] or 0) for row in rows}


def report_totals_for_month(month_value: str | None) -> dict[str, float]:
	linked_amounts = report_linked_amounts_for_month(month_value)
	fixed_amounts = report_fixed_amounts_for_month(month_value)
	fuel_adjustment = sum(fixed_amounts.get(key, 0) for key in REPORT_FIXED_DESCRIPTION_KEYS)
	totals: dict[str, float] = {}
	for description_key, _description_label in REPORT_DESCRIPTION_OPTIONS:
		if description_key in REPORT_FIXED_DESCRIPTION_KEYS:
			totals[description_key] = fixed_amounts.get(description_key, 0)
		elif description_key == "cost_of_sale_fuel":
			totals[description_key] = linked_amounts.get(description_key, 0) - fuel_adjustment
		else:
			totals[description_key] = linked_amounts.get(description_key, 0)
	return totals


def report_linked_accounts() -> dict[str, list[int]]:
	rows = query_all(
		"""
		SELECT description_key, account_id
		FROM report_account_links
		ORDER BY description_key, account_id
		"""
	)
	selected: dict[str, list[int]] = {}
	for row in rows:
		selected.setdefault(row["description_key"], []).append(row["account_id"])
	return selected
...
def report_fixed_amount_rows() -> list[sqlite3.Row]:
	return query_all(
		"""
		SELECT id, description_key, entry_date, amount
		FROM report_fixed_amounts
		ORDER BY description_key, entry_date DESC, id DESC
		"""
	)
...
def reports():
	...
	totals_by_description = report_totals_for_month(month_value)
	report_rows = [
		{
			"key": description_key,
			"label": description_label,
			"amount": totals_by_description.get(description_key, 0),
		}
		for description_key, description_label in REPORT_DESCRIPTION_OPTIONS
	]
	return render_template(
		"reports.html",
		title="Reports",
...
@app.route("/reports-settings", methods=["GET", "POST"])
def reports_settings():
	if request.method == "POST":
		action = request.form.get("action", "add_links").strip()
		description_key = request.form.get("description_key", "").strip()
		if action == "delete_fixed":
			entry_id = request.form.get("entry_id", type=int)
			if entry_id:
				execute("DELETE FROM report_fixed_amounts WHERE id = ?", (entry_id,))
		elif action == "update_fixed":
			entry_id = request.form.get("entry_id", type=int)
			entry_date = request.form.get("entry_date", "").strip()
			amount = parse_money(request.form.get("amount"))
			if entry_id and entry_date and amount is not None:
				execute(
					"UPDATE report_fixed_amounts SET entry_date = ?, amount = ? WHERE id = ?",
		...
		elif description_key in REPORT_FIXED_DESCRIPTION_KEYS:
			entry_date = request.form.get("entry_date", "").strip()
			amount = parse_money(request.form.get("amount"))
			if entry_date and amount is not None:
				execute(
					"INSERT INTO report_fixed_amounts (description_key, entry_date, amount) VALUES (?, ?, ?)",
					(description_key, entry_date, amount),
				)
		return redirect(url_for("reports_settings") + f"#{description_key}")
