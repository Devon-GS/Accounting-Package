...
def stage_workbook(
    ...
    try:
        ...
        return file_id, staged_rows, auto_resolved
    except Exception:
        temp_path.unlink(missing_ok=True)
        raise
...
def upload_cash_payments():
    ...
    for file_storage in files:
        ...
        if duplicate_flag:
            duplicates += 1
            flash(f"Skipped duplicate file: {Path(file_storage.filename).name}", "info")
            continue
    ...
    if imported_files == 0 and staged_rows == 0:
        ...
        flash(
            f"Imported {imported_files} file(s) and staged {staged_rows} payment row(s).",
            "success",
        )
        if duplicates:
            flash(f"{duplicates} duplicate file(s) were skipped.", "info")
